  ...
  emitter
    ...
    .on('response', ({ url, options, response }) => {
      ...
      t.equal(
        options.method,
        'GET',
        'should emit response options'
      )
      t.equal(
        response.status,
        200,
        'should emit response object'
      )
    })
    .on('response:body', ({ url, options, response, body }) => {
      ...
      t.equal(
        options.method,
        'GET',
        'should emit response:body options'
      )
      t.equal(
        response.status,
        200,
        'should emit response:body object'
      )
      t.equal(
        body.meta.type,
        'customerorder',
        'should emit response:body body'
      )
    })
    .on('error', err => {
