...
- ➕ (api) Добавление опции `retry` в параметры инициализации клиента.
- ➕ (api) Добавление статического метода `shouldRetryError`.
- ✏️ (docs) Дополнение документации. Исправление опечаток. Добавление примера кода для retry.
- 🧪 (tests) Запуск тестов и покрытия через GitHub Actions.

## 0.18.0 (2025-01-06)

- 🔧 (api) Опцию запроса `includeResponse` можно совмещать с опциями `muteApiErrors` и `muteCollectionErrors`, но не совместима с `rawResponse` и `rawRedirect`.
- ➕ (api) Добавление статического метода `getVersion`.
...
- ➕ (api) добавление возможности передавать тело запроса в виде уже сериализованной JSON строки
...
- ➕ (build) добавление минифицированной версии сборки `bundle/umd/moysklad.min.js`
...
- ➕ (buildUrl) вывод deprecated сообщения при передаче параметра `path` в виде массива
...
- ✏️ (getTimeString) удаление упоминания логики для корректировки часового пояса (глобальная переменная `MOYSKLAD_TIMEZONE`) добавленное в версии [0.8.0](#080) - по всей видимости не самое удачное решение
...
- ➕ (buildQuery) для описания поля `order` может использоваться нотация в виде массива, напр. `[['name', 'asc]]`
...
- 🔧 (filter) добавление возможности указывать значение фильтра как undefined (ключ фильтра в этом случаем будет опущен)
