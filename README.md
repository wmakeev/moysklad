...
- ⚠️ Исправлена ошибка: селектор `$exists` обрабатывался всегда как `!=`, вне зависимости от значения

### 0.4.0

- ⚠️ Исправлена ошибка: селектор `$lte` работал не верно (обрабатывался как `$gte`)
