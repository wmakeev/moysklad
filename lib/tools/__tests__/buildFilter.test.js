...
test('buildFilter with simple filter', t => {
  ...
  t.deepEqual(buildFilter(query), ['moment=2017-01-09 22:15:06', 'name=foo', 'other=true', 'some=', 'value=3'].join(';'));
...
test('buildFilter with simple deep and many condition filter', t => {
  ...
  t.deepEqual(buildFilter(query), ['deep.one=5', 'deep.tow=false', 'many=1', 'many=baz', 'moment=2017-01-09 22:15:06', 'name=foo', 'value=0'].join(';'));
...
test('buildFilter with mogo query comparison selectors', t => {
  ...
  t.deepEqual(buildFilter(query), ['cont~str', 'deep.tow!=bar', 'empty=', 'end=~psfx', 'many!=', 'many=1', 'many=baz', 'moment<=2017-01-09 22:15:06', 'name=foo', 'notMany!=3', 'notMany!=6', 'notMany>0', 'num<10', 'num>=5', 'start~=prfx', 'value>5'].join(';'));
...
test('buildFilter with mogo query logical selectors', t => {
  ...
  t.deepEqual(buildFilter(query), ['fantom=', 'name=bar', 'name=foo', 'value!=10', 'value!=5', 'value!=6'].join(';'));
